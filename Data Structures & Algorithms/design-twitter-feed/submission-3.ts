class Twitter {
    tweets: { user; tweet }[] = [];
    user2follows: Map<number, Set<number>> = new Map();
    ...
    getNewsFeed(userId: number): number[] {
        const follows = new Set(this.user2follows.get(userId)).add(userId);
        console.log(this.tweets);

        const result = [];
        for (let i = this.tweets.length - 1; i >= 0; i--) {
            const tweet = this.tweets[i];
            if (follows.has(tweet.user)) {
    ...
    follow(followerId: number, followeeId: number): void {
        const follows = this.user2follows.get(followerId) ?? new Set();
